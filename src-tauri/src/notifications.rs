...
//! - **desktop**: a native banner, but only when the notification preferences
//!   allow agent-board banners and the card is watched (`subscribed`).
//!
...
/// Fire-and-forget native desktop banner. macOS uses AppleScript; Linux uses
/// `notify-send` (libnotify/D-Bus). Other platforms no-op until they have a
/// supported notification backend.
pub fn desktop_banner(title: &str, body: &str) {
    #[cfg(target_os = "macos")]
    {
...
use crate::preferences::{
    notification_allowed, NotificationCategory, NotificationChannel, NotificationPrefs,
...
pub fn spawn_scanner(app: AppHandle) {
    tauri::async_runtime::spawn(async move {
        ...
        loop {
            ...
            let notification_prefs = state.preferences_snapshot().notifications.normalised();
            let mut center = state
                .notifications
                .lock()
                .unwrap_or_else(|e| e.into_inner());
            center.scan(&reg, &app, &notification_prefs);
        }
    });
}
...
pub enum NotificationKind {
    /// An agent moved the card to Done.
    Done,
    ...
    Warning,
    /// An agent recorded a project learning (a rule for `LEARNINGS.md`). Not
    /// tied to a card — clicking it opens the project's Learnings panel.
    Learning,
}
...
impl NotificationKind {
    ...
    fn banner_verb(self) -> &'static str {
        match self {
            ...
            NotificationKind::Done => "finished",
            NotificationKind::Learning => "recorded a learning for",
        }
    }

    fn severity(self) -> NotificationSeverity {
        match self {
            NotificationKind::Done => NotificationSeverity::Success,
            NotificationKind::Comment => NotificationSeverity::Info,
            NotificationKind::Warning => NotificationSeverity::Warning,
            NotificationKind::Blocked => NotificationSeverity::Error,
            NotificationKind::Learning => NotificationSeverity::Info,
        }
    }
}
...
pub struct Notification {
    ...
    /// The originating audit entry's stable id. For a `Comment` this is the
    /// comment's id, so a bell click can deep-link straight to that comment in
    /// the card's thread. `None` for kinds with no per-entry anchor.
    ...
    pub created_ms: u64,
    pub read: bool,
    /// The card's "Notify on agent activity" state when this fired. When true it
    /// forces the agent-event sound on the frontend even if Settings has that
    /// event's sound toggled off — a per-card override of the global pref.
    /// Defaults false so notifications persisted before this field round-trip.
    #[serde(default)]
    pub subscribed: bool,
}
...
impl NotificationCenter {
    ...
    /// agent activity. Native banners are gated by Settings → Notifications.
    #[cfg(feature = "tasks")]
    pub fn scan(&mut self, reg: &Registry, app: &AppHandle, prefs: &NotificationPrefs) {
        ...
        for project in reg.list_projects() {
            ...
            // Read id-carrying entries so a comment notification can deep-link to
            // the exact comment (its stable audit id), and resolve placeholder
            // agent ids back to the concrete kind seen on the same run.
            let recorded = match audit::read_recorded(path) {
                Ok(e) => audit::resolve_agent_identities(e),
                Err(_) => continue,
            };
            ...
            for rec in &recorded {
                ...
                // Resolve the card for a human title + watch state. A learning has
                // no card (empty id), so skip the lookup and leave the title blank
                // — the bell renders it as "<agent> recorded a learning". Otherwise
                // fall back to the id if the card was deleted between write + scan.
                let (card_title, subscribed) = if entry.card_id.is_empty() {
                    (String::new(), false)
                } else {
                    match board::read_card(path, &entry.card_id) {
                        Ok(pc) => (pc.card.title, pc.card.subscribed),
                        Err(_) => (entry.card_id.clone(), false),
                    }
                };
                // Carry the comment's stable id so a bell click can scroll to it;
                // other kinds have no per-entry anchor.
                let entry_id = if classified.kind == NotificationKind::Comment {
                    Some(rec.id.clone()).filter(|s| !s.is_empty())
                } else {
                    None
                };
                ...
                // A native banner is an interruption for when you're *away*. If
                // the app is the focused window the in-app bell + sound already
                // surface it where you're looking, so skip the desktop banner —
                // no point nudging you toward a window you're already in.
                if subscribed && !app_is_focused(app) && should_show_desktop(&notif, prefs) {
...
fn dedupe_id(project_id: &str, card_id: &str, at: &str, kind: NotificationKind) -> String {
    let action = match kind {
        ...
        NotificationKind::Learning => "learning",
    };
    format!("{project_id}|{card_id}|{at}|{action}")
}

/// Decide whether an audit entry is notable, and how. Returns `None` for
/// everything that isn't an agent comment, an agent block, an agent Done, or a
/// System warning.
#[cfg(feature = "tasks")]
fn classify(entry: &AuditEntry) -> Option<Classified> {
    ...
    match entry.action.as_str() {
        ...
        "move" if is_agent && entry.to == Some(BoardStatus::Done) => Some(Classified {
            kind: NotificationKind::Done,
            agent: agent_name,
            body: entry.note.clone(),
        }),
        // An agent appended a rule to LEARNINGS.md (recorded by `learning_add`).
        // The rule text rides in `note` for the bell preview; there's no card.
        "learning" if is_agent => Some(Classified {
            kind: NotificationKind::Learning,
            agent: agent_name,
            body: entry.note.clone(),
        }),
        // PortBay-recorded problem (e.g. acceptance check failed → Blocked).
        "note"
            if matches!(entry.actor, Actor::System) && entry.to == Some(BoardStatus::Blocked) =>
        {
            Some(Classified {
...
/// Whether PortBay's main window is the focused window. A `true` means the user
/// is already in the app, so a native desktop banner would be redundant noise.
/// Errors / a missing window resolve to "not focused" so we fail toward
/// notifying rather than swallowing an alert.
#[cfg(feature = "tasks")]
fn app_is_focused(app: &AppHandle) -> bool {
    app.get_webview_window("main")
        .and_then(|w| w.is_focused().ok())
        .unwrap_or(false)
}

/// Native desktop banner for a watched card, fire-and-forget.
#[cfg(feature = "tasks")]
fn notify_desktop(n: &Notification) {
    ...
    desktop_banner("PortBay", &body);
}

#[cfg(feature = "tasks")]
fn should_show_desktop(n: &Notification, prefs: &NotificationPrefs) -> bool {
...
mod tests {
    ...
    #[test]
    fn agent_done_is_notable_for_sound_and_bell() {
        let c = classify(&agent_entry("move", Some(BoardStatus::Done), None)).unwrap();
        assert_eq!(c.kind, NotificationKind::Done);
    }

    #[test]
    fn agent_learning_is_notable() {
        let mut e = agent_entry("learning", None, Some("Run `composer test`, not phpunit"));
        e.card_id = String::new(); // a learning has no card
        let c = classify(&e).unwrap();
        assert_eq!(c.kind, NotificationKind::Learning);
        assert_eq!(c.agent.as_deref(), Some("claude"));
        assert_eq!(c.body.as_deref(), Some("Run `composer test`, not phpunit"));
    }

    #[test]
    fn human_learning_is_silent() {
        // A learning recorded by a human edit isn't agent activity.
        let mut e = agent_entry("learning", None, Some("rule"));
        e.actor = Actor::Human;
        assert!(classify(&e).is_none());
    }

    #[test]
    fn ordinary_moves_are_silent() {
