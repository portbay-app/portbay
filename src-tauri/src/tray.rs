...
//!
//! Surface:
//! - A persistent menu-bar icon: the PortBay mark as a monochrome macOS
//!   *template image*, so the OS tints it to match the menu-bar theme
//!   (black on light bars, white on dark) automatically. The bitmap is
//!   static — aggregate health is conveyed by the tooltip and the
//!   popover, not by swapping icon colours.
...
//!   has changed. That call refreshes the tooltip — the icon bitmap is
//!   static — on the main thread via Tauri's main-thread guarantee on
...
//!
//! The icon is a single PNG at `icons/tray/icon-template.png`, rendered
//! from `icon-template.svg` by `scripts/generate-tray-icons.sh` and
//! embedded at compile-time via `include_bytes!` so a packaged .app never
//! has to look for it on disk.
...
/// Compile-time icon payload — the single PortBay mark, rendered from
/// `icons/tray/icon-template.svg` by `scripts/generate-tray-icons.sh`.
/// Embedded so the bundled .app cannot drift from the binary. It's a
/// monochrome template image: macOS tints it per menu-bar theme, so the
/// bitmap is identical for every status.
const ICON_TEMPLATE: &[u8] = include_bytes!("../icons/tray/icon-template.png");

fn tray_icon() -> Image<'static> {
    Image::from_bytes(ICON_TEMPLATE)
...
pub fn install(app: &AppHandle) -> tauri::Result<()> {
    ...
    let icon = TrayIconBuilder::with_id(TRAY_ID)
        .icon(tray_icon())
        .icon_as_template(true) // monochrome mark — macOS tints for light/dark menu bar
...
pub fn refresh(
    ...
    let (need_update, icon_clone) = {
        ...
        let need_update = tray.last_status != Some(aggregate);
        if need_update {
    ...
    // The icon bitmap is a static template (macOS tints it); only the
    // tooltip reflects status, so we refresh just that when the aggregate
    // changes.
    if need_update {
        let tooltip = format!(
            "PortBay — {} ({} project{})",
