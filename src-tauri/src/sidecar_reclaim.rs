...
impl SidecarKind {
    ...
    /// Every argv marker that identifies a PortBay-owned instance of this kind.
    /// A command line matches if it contains the [name marker](Self::name_marker)
    /// **and any one** of these. Most kinds have exactly one — the
    /// `<flag> <path>` launch shape from [`Self::config_marker`].
    ///
    /// php-fpm needs a second. After startup, PHP-FPM **rewrites its own
    /// process title** from the launch form `php-fpm -F -y <conf>` to
    /// `php-fpm: master process (<conf>)` — the `-y` flag is gone and the path
    /// is parenthesised. A boot-time reclaim only ever observes this rewritten
    /// title (the orphaned master has been running since the previous session),
    /// so matching solely on `-y …/PortBay/php` never fires: the orphan is
    /// never reaped, keeps its unix socket bound, and the fresh build's php-fpm
    /// dies at startup with "Another FPM instance seems to already listen…",
    /// surfacing as a spurious "php-fpm crashed" notification. The second
    /// marker — `master process (…/PortBay/php` — matches the rewritten title.
    /// It is just as safe as the first: it still carries the PortBay data-dir
    /// path (so a foreign ServBay/Homebrew FPM, whose conf lives elsewhere,
    /// can't match), and `master process (` is a shape no `grep`/`tail`/editor
    /// on the same file ever produces.
    pub fn config_markers(&self) -> Option<Vec<String>> {
        let primary = self.config_marker()?;
        Some(match self {
            SidecarKind::PhpFpm => {
                let php_dir = portbay_data_dir()?.join(self.config_file());
                vec![
                    primary,
                    format!("master process ({}", php_dir.to_string_lossy()),
                ]
            }
            _ => vec![primary],
        })
...
pub struct Signature {
    ...
    /// Launch-shape substrings (strong signal): the config path *as our daemon
    /// presents it*, flag included — unique to this install AND to the way we
    /// spawn it, so neither a foreign install nor a `tail`/`grep` on the same
    /// file can match. Usually one entry; php-fpm carries a second for its
    /// post-startup rewritten title (see [`SidecarKind::config_markers`]). A
    /// command line matches if it contains **any** of these.
    pub config_markers: Vec<String>,
}

impl Signature {
    /// A command line belongs to a PortBay-owned instance iff it carries the
    /// binary-name marker AND at least one of our config-path markers.
    pub fn matches(&self, command_line: &str) -> bool {
        command_line.contains(&self.name_marker)
            && self.config_markers.iter().any(|m| command_line.contains(m))
    }
}
...
mod tests {
  ...
  512     1 php-fpm: master process ({DATA}/php/8.4.21/php-fpm.conf)
  513     1 php-fpm: master process (/Applications/ServBay/etc/php/8.4/php-fpm.conf)
",
        )
    }
    ...
    fn sig(kind: SidecarKind) -> Signature {
        let primary = format!("{} {DATA}/{}", config_flag(kind), config_file(kind));
        let config_markers = match kind {
            // Mirror production: php-fpm also matches its rewritten master title.
            SidecarKind::PhpFpm => {
                vec![
                    primary,
                    format!("master process ({DATA}/{}", config_file(kind)),
                ]
            }
            _ => vec![primary],
        };
    ...
    fn matches_only_portbay_php_fpm_never_foreign() {
        ...
        assert_eq!(
            pids,
            vec![510, 512],
            "PortBay's php-fpm must match in both its launch form (510) and its \
             rewritten master-title form (512); ServBay's (511, 513) must not — \
             their pool config lives outside our data dir"
        );
    }

    // Regression guard for the boot-crash bug: PHP-FPM rewrites its own process
    // title after startup from `php-fpm -F -y <conf>` to
    // `php-fpm: master process (<conf>)`, dropping the `-y` flag. A boot reclaim
    // only ever sees this rewritten title, so the signature MUST match it — or
    // the orphaned master is never reaped, keeps its socket bound, and the fresh
    // build's php-fpm dies with "Another FPM instance seems to already listen".
    #[test]
    fn php_fpm_matches_rewritten_master_title() {
        let s = sig(SidecarKind::PhpFpm);
        // Our master, post-rewrite — must match (this is the bug).
        assert!(
            s.matches(&format!(
                "php-fpm: master process ({DATA}/php/8.4.21/php-fpm.conf)"
            )),
            "rewritten PortBay FPM master title must match"
        );
        // A foreign FPM's rewritten title must still never match — its conf
        // lives outside our data dir.
        assert!(
            !s.matches("php-fpm: master process (/Applications/ServBay/etc/php/8.4/php-fpm.conf)"),
            "foreign FPM master title must not match"
        );
        // A user tailing/grepping our conf must not be mistaken for the daemon:
        // it carries the path but neither the `-y` flag nor the `master process (`
        // title shape.
        assert!(
            !s.matches(&format!("/usr/bin/tail -f {DATA}/php/8.4.21/php-fpm.conf")),
            "a tail on the conf file must not match"
        );
    }
