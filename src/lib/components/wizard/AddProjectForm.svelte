<!--
  AddProjectForm — the add-project form body, decoupled from any one host.

  Rendered by two surfaces: `AddProjectWizard` (the global right-side rail) and
  the Domains page's inline "Add Domain" pane. Both mount this component while
  open and unmount it on close, so it owns no open/close state of its own — it
  takes a `mode` and reports completion through `onClose(createdId?)`.
  ...
  ESC + × button + Cancel all close. Unsaved input is preserved while the form
  stays mounted; closing discards.
-->
<script lang="ts">
  ...
  import type { WizardMode } from "$lib/stores/wizard.svelte";
  ...
  let {
    ...
    /** "project" = full add-project flow; "board" = folder + name only. */
    mode?: WizardMode;
    /** Override the header title (e.g. "New Domain"); defaults per mode. */
    heading?: string;
    /** Close the form; `createdId` is the new project's id on success. */
   ...
   */
  const isBoard = $derived(mode === "board");
  const title = $derived(heading ?? (isBoard ? "New board" : "Add project"));
  ...
  let languageIntelligence = $state<LanguageIntelligenceCapability[]>([]);
  ...
  function close(createdId?: string) {
    // Both hosts unmount the form on close, so a synchronous reset is enough —
    // the next open mounts a fresh instance.
    resetForm();
    onClose(createdId);
  }
  ...
  async function detectSingleFolder(folderPath: string) {
    ...
    kind = det.kind;
    languageIntelligence = det.languageIntelligence ?? [];
    syncRawFromFields();
  ...
  function chooseWorkspaceApp(app: WorkspaceApp) {
    ...
    languageIntelligence = app.languageIntelligence ?? [];
    formError = null;
    syncRawFromFields();
    schedulePortCheck(port);
  }
  ...
  async function commit() {
    ...
    try {
      if (portfile) {
        ...
        createdId = await safeInvoke<string>("import_portfile_commit", {
          input: {
            path,
            id: id || undefined,
        ...
        const added = await safeInvoke<ProjectView>("add_project", {
          input: {
            path,
            id: id || undefined,
            name: name || undefined,
...
<!-- Header / scrollable body / pinned footer. The host (rail aside or the
     Domains inline pane) supplies the surrounding chrome; Escape + the header
     close button + Cancel all dismiss it. -->
<div class="h-full min-h-0 flex flex-col">
    ...
    <div class="flex-1 min-h-0 overflow-y-auto p-5 space-y-4">
      ...
      {#if !isBoard && languageIntelligence.length > 0}
        <DashboardCard title="Smart editor support" flush>
          <div class="space-y-2">
            {#each languageIntelligence as item (item.language)}
              <div class="rounded-md border border-border/70 bg-bg/40 px-3 py-2">
                <div class="flex items-center justify-between gap-3">
                  <span class="text-[12.5px] font-medium text-fg">{item.label}</span>
                  <span class="text-[10px] uppercase tracking-wide text-fg-subtle">
                    {item.features.length} features
                  </span>
                </div>
                <p class="mt-1 text-[11px] text-fg-muted">{item.setup}</p>
                <p class="mt-1 truncate font-mono text-[10.5px] text-fg-subtle">
                  {item.files.join(", ")}
                </p>
              </div>
            {/each}
