...
PortBay is a local development environment manager. PortBay Cloud is an **optional** layer
that provides account sign-in, the Pro entitlement, and encrypted multi-device sync. You
can use PortBay fully without ever accepting these Terms (the anonymous tier).

By creating an account or using PortBay Cloud you agree to these Terms.
...
activity under your account and for keeping your sign-in method secure. One account per
person/identity; don't impersonate others.

## 4. Acceptable use

Don't: abuse the API (excessive automated requests), attempt to forge or tamper with
...
We may revoke a Pro entitlement for refund, reverted contribution, or abuse. **A
downgrade or revocation never deletes or disables your existing projects or data** — it
only blocks new gated actions. A previously-signed-in Pro user retains offline access
during a grace window (see the entitlement spec).
...
PortBay Cloud is provided on a reasonable-effort basis with **no uptime guarantee or
SLA**. Because PortBay is local-first, a Cloud outage does not stop the app from working
on your machine (Pro continues offline within its grace window).
...
We may update these Terms; material changes will be surfaced in-app and dated here.
